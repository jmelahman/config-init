...
so_R_u32_u32 bits_Div32(uint32_t hi, uint32_t lo, uint32_t y) {
    ...
    uint32_t quo = (uint32_t)(so_div(z, (uint64_t)(y))), rem = (uint32_t)(so_mod(z, (uint64_t)(y)));
...
so_R_u64_u64 bits_Div64(uint64_t hi, uint64_t lo, uint64_t y) {
    ...
    if (hi == 0) {
        return (so_R_u64_u64){.val = so_div(lo, y), .val2 = so_mod(lo, y)};
    }
...
uint32_t bits_Rem32(uint32_t hi, uint32_t lo, uint32_t y) {
    return (uint32_t)(so_mod((((uint64_t)(hi) << 32) | (uint64_t)(lo)), (uint64_t)(y)));
}
...
uint64_t bits_Rem64(uint64_t hi, uint64_t lo, uint64_t y) {
    ...
    so_R_u64_u64 _res1 = bits_Div64(so_mod(hi, y), lo, y);
