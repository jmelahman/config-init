...
typedef struct mem_atomicStats mem_atomicStats;
...
typedef struct mem_Allocator {
    void* self;
    so_R_ptr_err (*Alloc)(void* self, so_int, so_int);
    void (*Free)(void* self, void*, so_int, so_int);
    so_R_ptr_err (*Realloc)(void* self, void*, so_int, so_int, so_int);
} mem_Allocator;
...
//
typedef struct mem_atomicStats {
    atomic_Uint64 alloc;
    atomic_Uint64 totalAlloc;
    atomic_Uint64 mallocs;
    atomic_Uint64 frees;
} mem_atomicStats;
...
#define mem_TryAllocSlice(T, a_, len_, cap_) ({ \
    ...
    so_assert(_cap < so_div(so_max_int, _esize), "mem: capacity overflow"); \
    void* _ptr = NULL; \
    so_Error _err = {0}; \
    if (_cap > 0) { \
...
#define mem_TryReallocSlice(T, a_, slice_, newLen_, newCap_) ({ \
    ...
    so_assert(_newCap < so_div(so_max_int, _esize), "mem: capacity overflow"); \
    void* _newPtr = NULL; \
    so_Error _err = {0}; \
    if (_newCap == 0) { \
        if (_oldCap > 0) { \
