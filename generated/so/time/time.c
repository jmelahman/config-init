...
static time_Duration time_div(time_Time t, time_Duration d) {
    ...
    if (d < time_Second && so_mod(time_Second, (d + d)) == 0) {
        r = (time_Duration)(so_mod(nsec, (int32_t)(d)));
    } else if (d % time_Second == 0) {
        int64_t d1 = (int64_t)(d / time_Second);
        r = (time_Duration)(so_mod(sec, d1)) * time_Second + (time_Duration)(nsec);
    } else {
...
static so_R_int_int norm(so_int hi, so_int lo, so_int base) {
    if (lo < 0) {
        so_int n = so_div((-lo - 1), base) + 1;
        hi -= n;
        lo += n * base;
    }
    if (lo >= base) {
        so_int n = so_div(lo, base);
        hi += n;
        lo -= n * base;
    }
